...
export function PointLedgerTab(props: PointLedgerTabProps) {
  ...
  return (
    <Card>
      ...
      {ledger.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-1.5 text-center mb-2 text-xs">
          <div className="p-1.5 bg-emerald-50 rounded"><p className="font-bold text-emerald-700">{ledger.filter((i) => i.statusLabel === "Aman").length}</p><p>Aman</p></div>
          <div className="p-1.5 bg-amber-50 rounded"><p className="font-bold text-amber-700">{ledger.filter((i) => i.statusLabel === "Pembinaan ringan").length}</p><p>Pembinaan</p></div>
          <div className="p-1.5 bg-orange-50 rounded"><p className="font-bold text-orange-700">{ledger.filter((i) => i.statusLabel === "Panggilan orang tua").length}</p><p>Panggilan</p></div>
          <div className="p-1.5 bg-rose-50 rounded"><p className="font-bold text-rose-700">{ledger.filter((i) => i.statusLabel === "Kesiswaan/BK").length}</p><p>BK</p></div>
          <div className="p-1.5 bg-rose-100 rounded"><p className="font-bold text-rose-900">{ledger.filter((i) => i.statusLabel === "Tindak lanjut khusus").length}</p><p>Khusus</p></div>
        </div>
      )}
      {ledgerDetailStudent ? (
        <div className="space-y-2">
          ...
          {ledgerDetailStudent.totalPoints >= 50 && <div className="p-1.5 rounded bg-amber-50 text-xs text-amber-800">Rekomendasi: Surat Panggilan Orang Tua/Wali.</div>}
          {ledgerDetailStudent.totalPoints >= 75 && <div className="p-1.5 rounded bg-rose-50 text-xs text-rose-800">Rekomendasi lanjutan: koordinasikan dengan Kesiswaan/BK.</div>}
