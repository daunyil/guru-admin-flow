/**
 * LedgerDetailSheet — Dialog showing student's discipline ledger detail.
 *
 * Simple approach: fixed header + scrollable body. That's it.
 */
...
export function LedgerDetailSheet({
  ...
  return (
    <>
      ...
      {/* Centered dialog — SIMPLE: max-h + overflow-hidden on container, overflow-y-auto on body */}
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="flex justify-between items-center px-4 py-3 border-b border-slate-100 shrink-0">
            <h3 className="text-sm font-bold text-slate-800">Detail Siswa</h3>
            <button
              onClick={onClose}
              className="text-slate-400 font-bold text-lg w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100"
            >
              ✕
            </button>
          </div>
          ...
          <div className="overflow-y-auto p-4 space-y-3">
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                <h3 className="text-sm font-bold text-slate-900">{student.studentName}</h3>
                <p className="text-[10px] text-slate-500">
                  {student.classLabel}
                  {student.studentNumber ? ` · No. ${student.studentNumber}` : ""}
                </p>
              </div>
              <span className={`shrink-0 text-[10px] font-bold px-2.5 py-1 rounded-full ${statusColor.bg}`}>
                {statusLabel}
              </span>
            </div>
            ...
            <button
              ...
              className={`w-full font-bold text-xs py-3 rounded-xl flex items-center justify-center gap-2 active:scale-[0.98] transition-all min-h-[44px] ${
            ...
            <div className="rounded-xl border border-slate-200 overflow-hidden">
              <div className="bg-slate-50 px-3 py-2 border-b border-slate-100">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-700 uppercase tracking-wider">Riwayat Catatan</span>
                  <span className="text-[10px] text-slate-500">{sortedRecords.length} catatan</span>
                </div>
              </div>
              {sortedRecords.length === 0 ? (
                <div className="p-4 text-center text-sm text-slate-400">Belum ada catatan</div>
              ) : (
